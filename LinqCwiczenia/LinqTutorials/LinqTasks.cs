...
namespace LinqTutorials
{
    public static class LinqTasks
    {
        ...
        //Function to print everything from a IEnumerable
        public static void PrintAll(IEnumerable<object> result)
        {
        ...
        {
            IEnumerable<Emp> result = Emps.Select(e => e).Where(e => e.Job == "Backend programmer");
            return result;
        }
        ...
        {
            IEnumerable<Emp> result = Emps.Select(e => e).Where(e => e.Job == "Frontend programmer" && e.Salary > 100)
        ...
        {
            IEnumerable<Emp> result = Emps.Select(e => e).Where(e => e.Salary == Emps.Max(e => e.Salary));
            return result;
        }
        ...
        {
            IEnumerable<object> result = Emps.Select(e => new { e.Ename, e.Job, e.Deptno })
        ...
        {
            //IEnumerable<object> result = Emps.Select(e => new { Praca = e.Job, LiczbaPracownikow = Emps.Count(e2 => e2.Job == e.Job) }).GroupBy(Emps => Emps.Praca);
            //
            var result = Emps.GroupBy(e => e.Job).Select(e => new { Praca = e.Key, LiczbaPracownikow = e.Count() });
            return result;
        }
        ...
        {
            bool result = false;
            if (Emps.Count(e => e.Job == "Backend programmer") > 0)
            {
                result = true;
            }
        ...
        {
            Emp result = Emps.Select(e => e).Where(e => e.Job == "Frontend programmer").OrderByDescending(e => e.HireDate).First();
            return result;
        }
        ...
        {
            IEnumerable<object> result = Emps.Select(e => new { e.Ename, e.Job, e.HireDate }).Union(Emps.Select(e => new { Ename = "Brak wartości", Job = (string)null, HireDate = (DateTime?)null }));
            return result;
        }
        ...
        {
            IEnumerable<object> result = Depts.Select(d => new { name = d.Dname, numOfEmployees = Emps.Select(e => e).Where(e => e.Deptno == d.Deptno).Count() }).Where(d => d.numOfEmployees > 1);
            return result;
        }
        ...
        {
            IEnumerable<Dept> result = Depts.Select(d => d).Where(d => Emps.Count(e => e.Deptno == d.Deptno) == 5 ||
            Emps.Count(e => e.Deptno == d.Deptno) == 0).OrderBy(d => d.Dname);
            //result =
            return result;
        }
        ...
        {
            //IEnumerable<object> result = Emps.Select(e => e).Where(e => e.Job.Contains("A")).GroupBy(e => e.Job).Where(e => e.Count() > 2).OrderByDescending(e => e.Count()).SelectMany(e => e);
            //
            var result = Emps.GroupBy(e => e.Job).Select(e => new { Praca = e.Key, LiczbaPracownikow = e.Count() }).Where(e => e.Praca.Contains("A") && e.LiczbaPracownikow > 2).OrderByDescending(e => e.LiczbaPracownikow);
        ...
        {
            var result = Emps.Select(e => new { e.Ename, e.Job, e.Deptno }).Join(Depts, e => true, d => true, (e, d) => new { e, d });
            //result =
            return result;
        }
    }
